...
7. 读取到的信息必须包含角色 ID 和服务器，支持同一玩家多个角色分别排查商城时装、图鉴和仓库状态。
...
| 多角色档案 | 同一玩家可按角色分别排查收集状态 | snapshot 使用角色 ID + 服务器作为身份键，支持角色切换与对比 |
...
| 角色配置 | 当前数据属于哪个角色，如何管理多号 | 角色 ID + 服务器、本地角色档案、上次更新时间、数据来源、手动重命名显示、改名/转服后的手动合并入口 |
...
| 数据契约 | `src/lib/armoire/types.ts`、`normalizeSnapshot.ts`、helper `Models.cs` | 当前文档已要求 `character.id`，代码仍需补齐；这是实现三分区前的前置小改。 |
...
1. 先补齐 snapshot 角色身份代码契约：`character.id`、校验、示例 snapshot、helper 输出预留。
...
7. 多角色场景必须优先读取并保留 `character.id` 和 `character.world`；角色名只作为显示字段，不能单独作为跨 snapshot、商城时装统计或图鉴统计的匹配依据。
8. 如果某个来源暂时无法提供稳定角色 ID，页面必须把该 snapshot 标记为“身份不完整”的导入数据，只能做当前快照分析，不能和其他角色档案自动合并。
...
1. 补齐前端和 helper snapshot 契约中的 `character.id` 字段，normalize 和示例数据同步更新。
...
7. 商城时装、图鉴和仓库统计都要按角色 ID + 服务器归档，支持用户在多个角色之间切换和对比，避免不同角色的购买/拥有状态混在一起。
...
- 需要读取角色 ID 和服务器；多角色档案、商城时装统计和图鉴查询都必须按“角色 ID + 服务器”区分。
