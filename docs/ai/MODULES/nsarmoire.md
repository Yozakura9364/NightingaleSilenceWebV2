---
summary: "衣柜管家公开下载入口、本地工作台、helper、snapshot 和 catalog 边界。"
status: "active"
scope: "公网 armoire 页面、armoire-local 构建和 NSArmoireButler 集成。"
source_of_truth: "src/pages/armoire、docs/api/nsarmoire.md、helper 与 catalog scripts。"
read_when: "修改衣柜页面、helper、缓存、分析、catalog 或发布边界。"
update_when: "页面分工、数据契约、helper 能力、存储或构建模式变化时。"
verify: "分别检查公网构建、本地构建、helper 状态和代表性 snapshot。"
---
...
| 层级       | 涉及文件                                                                 | 评估                                                                                             |
| ---------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| 页面外壳   | `NSArmoireWorkspace.vue`                                                 | 已从纵向信息流改为“分区导航 + active section”容器，并保留 import/helper/catalog 组合状态。       |
| 新导航组件 | `NSArmoireSectionRail.vue`                                               | 已建立页面私有左侧图标轨；文案和 aria-label 走 Armoire 本地化 key。                              |
| 衣柜清理区 | `NSArmoireInsightPanel.vue`、`NSArmoireOverview.vue`                     | 已复用处理建议并调整组合位置和摘要密度。                                                         |
...
| 本地化     | `src/config/site.ts`、`src/locales/keys/armoire.ts`、`src/locales/modules/armoire.ts` | 三分区标题、导航 aria、角色配置字段和状态文案统一走 key。                     |
...
早期曾同时设计 Vite 开发代理和公网网页直连本机 Helper 两种模式。该浏览器直连方案已经废止，当前边界为：

1. 开发代理模式：Vite 把 `/api/armoire/*` 代理到本机 Helper，便于开发。
2. 正式本地模式：`NSArmoireButler` 的 WPF/WebView2 GUI 内嵌 `armoire-local` 构建，页面、catalog 和 API 均由 `127.0.0.1:8015` 同源提供。
3. 公网模式：`#/ffxiv/armoire` 只提供教程和 GitHub Release 下载入口，不读取 snapshot、不连接 Helper、不发布 Armoire catalog。

手动 JSON 导入只保留在本地完整工作台中，作为调试、兼容和故障 fallback。
...
该段属于早期预研记录。当前正式路线已经确定为独立 `NSArmoireButler` + WPF/WebView2 内嵌本地工作台；Dalamud 插件仍是后续可选预研方向。
...
第一版本地 Helper 接入后，`src/config/site.ts`、`src/services/apiBoundaries.ts` 和 `vite.config.ts` 已补充 `/api/armoire` 开发代理。当前正式完整工作台由 Helper GUI 内嵌并通过 `http://127.0.0.1:8015` 同源运行；公网页面不再承担 Helper 连接。
...
1. 新增本地助手，已从 V2 内置 `tools/nsarmoire-helper` 拆为仓库同级独立项目 `../NSArmoireButler`，使用 C# / .NET 7 构建。
...
1. `/icon/:itemId` 暂未实现。
2. Helper GUI 内嵌工作台的同源启动、刷新和升级链路仍需持续回归。
...
| Helper 内嵌工作台兼容性      | WebView2 运行时、Helper 版本和本地静态构建可能出现版本不一致                              | Release 固定配套构建，回归同源启动、刷新、升级和手动导入 fallback                                       |
...
- [x] 当时确认 V2 尚无 `NSArmoire` 路由和模块代码，随后已完成接入。
...
- [ ] Helper GUI 内嵌工作台可同源连接 API，公网下载页不请求 Helper 或 Armoire catalog

## 当前剩余事项

1. 继续验证游戏版本更新后的内存签名、容器语义和失败提示。
2. 取得稳定角色 ID 前，不自动合并改名或转服后的角色档案。
3. 持续校正商城、收藏柜、套装、同模型和染剂 catalog，并保持生成器/checker 可复现。
4. Dalamud 插件如需启动，必须另开独立项目并先完成官方开发指南、读取边界、分发和安全评估。
5. Helper Release 必须固定配套 `armoire-local` 构建，并回归 WebView2 同源启动、刷新、升级和手动导入 fallback。
