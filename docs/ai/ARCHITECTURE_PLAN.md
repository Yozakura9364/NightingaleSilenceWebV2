...
- 当前不是所有旧项目后端的完整替代品；`NSPlate` 和 `NSGlamour` 已具备核心工作台，前者默认脱离旧服务，后者仍通过旧 Flask 契约运行。其他模块按各自模块文档描述。
...
| `#/ffxiv/glamour` | `NSGlamour` 幻化工具                        | EquipInfo/Template 双工作台已接入，当前通过旧 Flask 契约运行 |
