---
summary: "V2 的项目身份、技术栈、真实模块状态、目录结构和目标边界。"
status: "active"
scope: "整站 Vue 前端及 NSPlate、NSGlamour、物品卡片、时尚品鉴、NSArmoire、Silence、About。"
source_of_truth: "src/router、src/config/site.ts、package.json 和各模块文档。"
read_when: "每次新会话、跨模块修改、项目状态评估或迁移规划。"
update_when: "技术栈、路由、页面状态、数据源或模块边界变化时。"
verify: "对照 router、site config、package scripts 和 MODULE_MAP。"
---
...
| 构建工具 | Vite 6                                                                        |
| 语言     | TypeScript 5.7                                                                |
| 路由     | Vue Router 4，当前使用 hash mode；普通生产构建只注册公开页面，开发环境另注册 Silence、Style Lab 和术语校对路由 |
| 状态管理 | Pinia 已注册；`src/stores/locale.ts` 和 `src/stores/theme.ts` 以模块级响应式状态管理语言与明暗模式 |
| CSS      | 已建立第一版 `src/styles/` 公共样式体系                                       |
| 页面     | 已建立首页、FFXIV 分类页、物品卡片、时尚品鉴、NSPlate、NSGlamour 双工作台、NSArmoire 公网下载页/本地工作台、Silence 本地开发页面和 About 个人与友链页 |
...
| `NSGlamour` | EquipInfo 与 Template 双工作台及 Flask API 均已迁入：共享 `GlamourDraft`、网页链接/文字/隐式本地配置导入、装备候选替换、染剂编辑、多语言、最近记录、六套模板 Canvas、图片裁剪/最近图片暂存与原尺寸 PNG 导出均已接入。V2 使用本仓库 `server/glamour/` 的 `8766` 服务；旧 `8765` 仅在生产切流观察完成前作为回滚入口。 |
| `NSArmoire` | 完整工作台改由 `NSArmoireButler` 的 WPF/WebView2 GUI 本地承载；公网路由只保留教程和 GitHub Release 下载入口，公网构建不发布 Armoire catalog。 |
| `时尚品鉴` | 已建立当前周方案、作业下方的当周金牌物品、金牌查询和来源页面；公开页面读取经确认的当前周切片、中英日韩名称索引与不含证据的聚合标签索引，原始历史和参考继续保留在非公开数据边界。 |
| `物品卡片` | 已建立独立 `#/ffxiv/item-card` 工作台，支持网页链接或装备文本导入、装备/染剂编辑、文字样式、多语言、简易/完整版、单张 PNG、批量 ZIP 和连续列表长图；草稿自动保存，最近记录只保留兼容数据而不显示为主入口。不包含人物图片或幻化模板，页面源码和浏览器状态不与 V2 glamour 页面互相引用。 |
| `Silence` | 入口、分组和部分角色详情代码已接入；资料和正式素材尚未填写完成，仅开发环境默认注册，普通生产构建不生成路由或 chunk。 |
| 首页 / About | 首页是第一版视觉入口；About 已接入像素头像、SNS、本站信息和静态友链列表。 |
...
3. `src/App.vue` 初始化语言和主题，并在非 Armoire 本地构建中渲染轻量 `AppTopNav`，随后渲染 `<router-view />`。
...
| `#/ffxiv/armoire` | 衣柜管家          | 公网教程/下载页；完整工作台只进入 Helper 本地构建 |
| `#/ffxiv/fashioncheck` | 时尚品鉴      | V2 当前周公开作业页                 |
| `#/ffxiv/item-card` | 物品卡片        | 旧 `NSGlamour /card` 功能的独立 Vue 重建 |
| `#/silence`       | Silence 创作入口  | 代码已接入，资料未完成，暂不上线     |
| `#/silence/angel` | 不语·silence 分组 | 分组舞台和详情代码已接入，暂不上线   |
| `#/silence/glitch` | 幽灵·silence 分组 | 双人页代码已接入，暂不上线           |
| `#/about`         | 个人与友链页      | V2 新增                             |
