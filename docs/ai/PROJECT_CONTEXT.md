...
| `NSGlamour` | EquipInfo 与 Template 双工作台已迁入：共享 `GlamourDraft`、网页链接/文字/隐式本地配置导入、装备候选替换、染剂编辑、多语言、最近记录、六套模板 Canvas、图片裁剪/最近图片暂存与原尺寸 PNG 导出均已接入。当前仍依赖旧 Flask API；生产部署、后端替换和旧项目等价回归继续按模块文档推进。 |
