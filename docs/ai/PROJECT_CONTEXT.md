...
| 构建工具 | Vite 6                                                                        |
| 语言     | TypeScript 5.7                                                                |
| 路由     | Vue Router 4，当前使用 hash mode，已注册首页、FFXIV、NSPlate、NSGlamour、NSArmoire 轻量下载教程页、Silence 入口/分组/角色路由和 About |
| 状态管理 | Pinia 已安装；目前只有 `src/stores/locale.ts` 这种模块化 store 写法           |
| CSS      | 已建立第一版 `src/styles/` 公共样式体系                                       |
| 页面     | 已建立 `src/pages/` 下的首页、FFXIV 分类页、NSPlate 工作台、NSGlamour 的 EquipInfo/Template 双工作台、NSArmoire 第一阶段页、Silence 页面和 About 占位页 |
