...
status: 'active'
scope: 'server/shortlinks、/go/<code>、AstrBot 私聊管理命令和生产 Nginx/systemd。'
...
- 禁止用户名/密码型 URL，也禁止跳回本站 `/go/`，避免凭据泄露和短链循环。
...
- 生产站点由 1Panel OpenResty 容器加载 `/www/wwwroot/NightingaleSilenceWeb/conf.d/NSHome.conf`；该 server block 直接反代 `/go/` 到 `172.17.0.1:18768`。
