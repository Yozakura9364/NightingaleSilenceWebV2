...
独立的私人短链服务。公网只暴露 `GET https://nsffxiv.com/s/<code>`；`/internal/short-links` 管理接口必须使用 Bearer Token，且只应监听 Docker 网桥或环回地址。
...
生产公网路由位于 `/www/wwwroot/NightingaleSilenceWeb/conf.d/NSHome.conf`，由 1Panel OpenResty 容器加载。公网必须只代理 `/s/`，并显式让 `/internal/short-links` 返回 `404`。旧 `/go/` 不再保留。
