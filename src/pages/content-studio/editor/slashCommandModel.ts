...
export const SLASH_ITEMS: SlashItem[] = [
  ...
  {
    id: 'heading3',
    labelKey: key('heading3'),
    keywords: ['heading', 'h3', '标题', '三级'],
    run: (editor, range) => {
      editor.chain().deleteRange(range).setNode('heading', { level: 3 }).run()
    }
  },
  {
    id: 'bulletList',
    labelKey: key('bulletList'),
  ...
  },
  {
    ...
    run: (editor, range) => {
      editor.chain().deleteRange(range).insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()
    }
  },
  {
    id: 'image',
    ...
    opensDialog: true,
    run: (editor, range) => {
      editor.chain().deleteRange(range).insertContent({ type: 'image', attrs: {} }).run()
    }
  },
  {
    id: 'gallery',
    labelKey: key('gallery'),
...
export function filterSlashItems(query: string): SlashItem[] {
  ...
  return SLASH_ITEMS.filter(
    (i) => i.id.toLowerCase().includes(q) || i.keywords.some((k) => k.toLowerCase().includes(q))
  )
}
